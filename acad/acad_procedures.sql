...
DROP PROCEDURE cargarCarrera;
DROP PROCEDURE cargarPlanDeEstudio;
DROP PROCEDURE cargarPlanMateria;
DROP PROCEDURE matricularAlumno;
DROP PROCEDURE inscripcionCursada;
DROP PROCEDURE inscripcionExamen;
DROP PROCEDURE cargarParcial;
DROP PROCEDURE cargarNota;
...
proc_label: BEGIN
  ...
  IF isInscripto = 0 THEN
    SELECT 'El alumno no está inscripto en el examen';
    LEAVE proc_label;
  END IF;


  SELECT ie.nota INTO hasNota
  FROM Alumno a 
  JOIN Inscripcion_examen ie ON a.id_alumno = ie.id_alumno
  JOIN Turno_examen te on ie.id_turno_examen = te.id_turno_examen
  WHERE a.id_alumno = idAlumno AND te.id_materia = idMateria
